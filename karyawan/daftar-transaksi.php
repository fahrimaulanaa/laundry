...
<body>
        ...
        <div class="w-1/4 p-4">
            <div class="bg-white border rounded shadow">
                ...
                <div class="p-5">
                    <?php
                    include "../connect.php";
                    $sql = "SELECT * FROM  transaksi WHERE status = 'proses' or status = 'pencucian' or status = 'pengeringan' or status = 'setrika'";
                    ...
                    <div class="flex items-center justify-between mt-4">
                        <a href="tambah.php" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full" target="blank">Cek Transaksi</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="w-1/4 p-4">
            <div class="bg-white border rounded shadow">
                ...
                <div class="p-5">
                    <?php
                    include "../connect.php";
                    $sql = "SELECT * FROM  transaksi WHERE status = 'belum diproses'";
                    $result = mysqli_query($koneksi, $sql);
                    $resultCheck = mysqli_num_rows($result);
                    ?>
                    <h3 class="text-3xl text-gray-700 font-bold leading-none mb-3"><?php echo $resultCheck; ?></h3>
    ...
    <br>
    <hr>
    <!-- table for transaksi berlangsung -->
                ...
                <div class="p-5">
                    <table class="w-full p-5 text-gray-700" rowspan="7" colspan="7">
                        ...
                        <tbody>
                            <?php
                            include "../connect.php";
                            $sql = "SELECT * FROM transaksi WHERE status = 'pencucian' OR status = 'pengeringan' OR status = 'pengeringan' OR status = 'pengambilan'";
                            $result = mysqli_query($koneksi, $sql);
                            $resultCheck = mysqli_num_rows($result);
                            if ($resultCheck > 0) {
                                $no = 1;
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    echo "<td><a href='edit-transaksi.php?id_transaksi=" . $row['id_transaksi'] . "' class='  text-black font-bold py-2 px-4 rounded-full' target='blank'>Edit Transaksi</a></td>";
                                    echo "</tr>";
                                    $no++;
                                }
                            }
                        ...
                        <tbody>
                            ...
                            if ($resultCheck > 0) {
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    echo "<td>" . $row['total_transaksi'] . "</td>";
                                    echo "<td>" . $row['status'] . "</td>";
                                    echo "<td><a href='edit.php?id=" . $row['id_transaksi'] . "' class='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full' target='blank'>Proses</a> | <a href='delete.php?id=" . $row['id_transaksi'] . "' class='bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-full' target='blank'>Delete</a></td>";
                                    echo "</tr>";
                                    $no++;
                                }
                            }
